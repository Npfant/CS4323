...
void time(event){
    ...
    pthread_mutex_lock(&time_mutex); //Acquire lock
        if(event.type == 'REQ') sim_time++; //Event types of request and release increment time by 1, traversals by 2.
        ...
        sec = sim_time % 60;
        fp = fopen("sim.log", "a");
        if(event.type == 'REQ'){ //Request log statement
        ...
        else if(event.type == 'GRNT_MUT'){ //Mutex grant log statement
        ...
        else if(event.type == 'GRNT_SEM'){ //Semaphore grant log statement
        ...
        else if(event.type == 'QUE'){ //Queue log statement
        ...
        else if(event.type == 'REL'){ //Release log statement
        ...
        else if(event.type == 'DEAD'){ //Deadlock log statement
        ...
        else if(event.type == 'PRE'){ //Preemption log statement
        ...
        else if(event.type == 'COM'){ //Completion log statement
...
int main() {
    ...
    pthread_create(&threads[i], NULL, time, NULL);            
